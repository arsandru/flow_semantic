...
library(performance)
library(ggplot2)
data <- read.csv("semantic_projection_roberta.csv", stringsAsFactors = FALSE)
...
ggsave("semantic_projection_final.pdf", plot = p, width = 10, height = 8)
...
write.csv(coef_mixed_df, "coefficients_mixed_cr2.csv", row.names = FALSE)
write.csv(coef_lm_df, "coefficients_lm_cr2.csv", row.names = FALSE)
write.csv(comparison_check, "pairwise_comparison_mixed_vs_lm_cr2.csv", row.names = FALSE)
write.csv(sig_df, "significant_pairwise_findings.csv", row.names = FALSE)
