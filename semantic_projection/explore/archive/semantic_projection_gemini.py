...
def normalize_text(w: str) -> str:
    ...
    if key == "tune_out":
        return "tune out"
    #if key == "distracted":
    #    return "distracted from worries"
...
def _values_from_embedding_obj(obj) -> np.ndarray:
    if hasattr(obj, "values") and obj.values is not None:
        return np.array(obj.values, dtype=float)
    if isinstance(obj, dict) and "values" in obj:
        return np.array(obj["values"], dtype=float)
    raise ValueError("Unexpected Gemini embedding object format.")


def embed_texts(texts):
    normalized = [normalize_text(t) for t in texts]
    missing = [t for t in normalized if t not in embedding_cache]

    if missing:
        formatted = [format_input(t) for t in missing]
        ...
        if embeddings is None:
            ...
            if single is None:
                raise ValueError("Gemini response did not contain embedding data.")
            embeddings = [single]

        for text, emb in zip(missing, embeddings):
            embedding_cache[text] = _values_from_embedding_obj(emb)

    return [embedding_cache[t] for t in normalized]


def encode_word(w: str) -> np.ndarray:
    return embed_texts([w])[0]
...
all_normalized_words = sorted({
    normalize_text(w)
    for word_list in merged["text"]
    if isinstance(word_list, list)
    for w in word_list
    if isinstance(w, str)
})
if all_normalized_words:
    embed_texts(all_normalized_words)
...
word_level_df.to_csv(OUTPUT_DIR / "semantic_projection_gemini.csv", index=False)
participant_level_df.to_csv(OUTPUT_DIR / "semantic_projection_gemini_mean.csv", index=False)
...
plot_df.to_csv(OUTPUT_DIR / "semantic_projection_gemini_plot_points.csv", index=False)
...
label_df.to_csv(OUTPUT_DIR / "semantic_projection_gemini_plot_labels.csv", index=False)
...
plt.title("Semantic Projection of Emotional Words on Distress-Relaxed Axis (Gemini)", fontsize=18)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "semantic_projection_gemini.pdf", format="pdf")
plt.savefig(OUTPUT_DIR / "semantic_projection_gemini.svg", format="svg")
plt.show()
