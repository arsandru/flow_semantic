...
def embed_texts(texts, batch_size=16):
    ...
    if missing:
        ...
        for text, vector in zip(missing, vectors):
            embedding_cache[text] = np.asarray(vector, dtype=float)
...
word_level_df.to_csv(OUTPUT_DIR / "semantic_projection_jina.csv", index=False)
participant_level_df.to_csv(OUTPUT_DIR / "semantic_projection_jina_mean.csv", index=False)
...
plot_df.to_csv(OUTPUT_DIR / "semantic_projection_jina_plot_points.csv", index=False)
...
label_df.to_csv(OUTPUT_DIR / "semantic_projection_jina_plot_labels.csv", index=False)
...
plt.title("Semantic Projection of Emotional Words on Distress-Relaxed Axis (Jina v5 text-small)", fontsize=18)
...
plt.tight_layout()
plt.savefig(OUTPUT_DIR / "semantic_projection_jina.pdf", format="pdf")
plt.savefig(OUTPUT_DIR / "semantic_projection_jina.svg", format="svg")
plt.show()
